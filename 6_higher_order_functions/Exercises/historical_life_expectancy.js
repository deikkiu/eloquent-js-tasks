...
const a = (obj) => {
  ...
  obj.forEach((el) => {
    let { born, died } = el;
    let key = Math.ceil(died / 100);
    let value = Math.round(died - born);
    return (b[key] ||= []).push(value);
  });

  return b;
};

function sumOfYear(obj) {
  for (let [key, value] of Object.entries(obj)) {
    obj[key] = Number(
      (value.reduce((a, b) => a + b, 0) / value.length).toFixed(1)
    );
  }
  return obj;
}

console.log(sumOfYear(a(ancestry)));
