...
function getOld(array) {
  ...
  for (let i = 0; i < array.length; i++) {
    ...
    if (current.died < 1600) sixteen.push(current);
    if (current.died < 1700) seventeen.push(current);
    if (current.died < 1800) eigthteen.push(current);
    if (current.died < 1900) nineteen.push(current);
    if (current.died < 2000) twenty.push(current);
    if (current.died < 2100) twentyone.push(current);
  }
...
function oldPerson(array) {
  for (let i = 0; i < array.length; i++) {
    let current = array[i];
    let count = current.died - current.born;
  }
}

console.log(oldPerson(getOld(ancestry)));
